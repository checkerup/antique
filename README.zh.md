...
📚 **知识库：** [`docs/wiki/KB-INDEX.md`](docs/wiki/KB-INDEX.md) —— 按领域版本化文档（API、安全、部署、代理、指纹、UI）。更新日志：[`CHANGELOG.md`](CHANGELOG.md)。
...
- 不是多用户鉴权层 —— 单进程，单一 Bearer 令牌（`ANTIQUE_API_TOKEN`；本地回环默认关闭，`--deploy-mode lan` 下必填）。
