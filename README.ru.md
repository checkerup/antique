...
antidetect-local — это Python-сервис, который:
...
- Сохраняет профили в SQLite (`data/antidetect.db`) — proxies, fingerprints, cookies, tags, sessions, import bookkeeping.
...
База данных: `data/antidetect.db` (SQLite, один файл).
...
Override через `ANTIDETECT_DATA_DIR=/some/path` (env var).
