...
# antique — почти бесплатный антидетект-браузер

**Antique — почти бесплатный антидетект-браузер. Самохостируемый open-source аналог платных подписок AdsPower — многопрофильная ферма браузеров с подменой fingerprint, ротацией proxy, импортом `.adb`-бандлов и AdsPower-совместимым REST API.**

📚 **База знаний:** [`docs/wiki/KB-INDEX.md`](docs/wiki/KB-INDEX.md) — версионируемые доки по всем доменам. Список изменений: [`CHANGELOG.md`](CHANGELOG.md).

> Собран автономно, чтобы заменить платную подписку AdsPower тем же UX и формой API, без лицензий, полностью локально. Платите только за прокси — сам софт ничего не стоит.
...
- Не мультипользовательский auth-слой — однопроцессный, один Bearer-токен (`ANTIQUE_API_TOKEN`; на loopback по умолчанию выключен, при `--deploy-mode lan` обязателен).
